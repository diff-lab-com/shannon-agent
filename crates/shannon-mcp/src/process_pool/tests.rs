...
use super::types::{
    MAX_TOOL_DESCRIPTION_CHARS, MAX_TOOL_RESULT_CHARS, PendingRequest, ServerState, glob_match,
    is_tool_allowed_by_patterns, normalize_error_content, truncate_tool_result,
};
use super::*;
use dashmap::DashMap;
...
fn test_deferred_schema_store_and_retrieve() {
    ...
    assert!(
        pool.deferred_schema_tool_names()
            .contains(&"mcp__test__tool".to_string())
    );
}
...
fn test_tool_description_truncation() {
    ...
    assert!(
        desc.chars().count() <= MAX_TOOL_DESCRIPTION_CHARS + 1,
        "description should be truncated to ~2048 chars"
    );
...
fn test_tool_result_truncated_json() {
    let items: Vec<String> = (0..5000)
        .map(|i| format!(r#"{{"id": {i}, "data": "item {i}"}}"#))
        .collect();
...
fn test_normalize_error_content_text_only() {
    let blocks = vec![serde_json::json!({"type": "text", "text": "Error: file not found"})];
...
fn test_normalize_error_content_unknown_type() {
    let blocks = vec![serde_json::json!({"type": "audio", "data": "..."})];
    let result = normalize_error_content(&blocks);
...
async fn test_pool_allowed_patterns() {
    ...
    pool.set_allowed_patterns(vec!["mcp__fetch__*".to_string()])
        .await;
...
fn test_enforce_output_limit_preserves_unicode() {
    ...
    // Should not panic on char boundary
    assert!(
        limited.content.contains("[compressed:")
            || limited.content.contains("chunk_id=")
            || limited.content.len() <= 200
    );
}
...
async fn test_tool_result_store_roundtrip() {
    ...
    let (tool_name, full) = pool
        .get_stored_result(&chunk_id)
        .await
...
async fn test_tool_result_store_chunking() {
    ...
    // Get first chunk.
    let chunk = pool
        .get_result_chunk(&chunk_id, 0, 100)
        .await
    ...
    // Get second chunk.
    let chunk2 = pool
        .get_result_chunk(&chunk_id, chunk.offset, 100)
        .await
    ...
    // Get beyond end.
    let chunk_end = pool
        .get_result_chunk(&chunk_id, 2000, 100)
        .await
...
fn test_progress_notification_deserialization() {
    ...
    let notif: crate::ProgressNotification = serde_json::from_value(json).unwrap();
...
async fn test_roots_provider_default_none() {
    ...
    assert!(
        roots.is_empty(),
        "default roots provider should return empty vec"
    );
}
...
fn test_create_message_request_deserialization() {
    ...
    let req: crate::CreateMessageRequest = serde_json::from_value(json).unwrap();
...
async fn test_budget_enforcement() {
    ...
    assert_eq!(
        pool.server_total_result_bytes("budget-srv").await,
        Some(800)
    );
