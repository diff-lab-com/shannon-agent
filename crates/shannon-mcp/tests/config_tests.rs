...
use shannon_mcp::config::{
    ConfigError, HeaderSource, McpAuthConfig, McpConfig, McpServerConfig, discover_config,
    expand_env_vars,
};
...
fn test_parse_mcp_config_with_multiple_servers() {
    ...
    let local =
        McpServerConfig::from_json_value(servers_obj.get("local-files").unwrap().clone()).unwrap();
    assert!(matches!(local, McpServerConfig::Stdio { .. }));

    let remote =
        McpServerConfig::from_json_value(servers_obj.get("remote-api").unwrap().clone()).unwrap();
    assert!(matches!(remote, McpServerConfig::Sse { .. }));

    let ws =
        McpServerConfig::from_json_value(servers_obj.get("ws-server").unwrap().clone()).unwrap();
...
fn test_expand_bare_env_var() {
    unsafe {
        std::env::set_var("TEST_MCP_CONFIG_PATH", "/usr/local/bin");
    }
    let result = expand_env_vars("$TEST_MCP_CONFIG_PATH/tool");
    assert_eq!(result, "/usr/local/bin/tool");
    unsafe {
        std::env::remove_var("TEST_MCP_CONFIG_PATH");
    }
}
...
fn test_expand_default_value_when_missing() {
    unsafe {
        std::env::remove_var("TEST_MCP_CONFIG_ABSENT");
    }
...
fn test_expand_missing_var_yields_empty() {
    unsafe {
        std::env::remove_var("TEST_MCP_CONFIG_GONE");
    }
...
fn test_expand_multiple_vars_in_string() {
    unsafe {
        std::env::set_var("TEST_MCP_CONFIG_PROTO", "https");
    }
    unsafe {
        std::env::set_var("TEST_MCP_CONFIG_SVC", "api.example.com");
    }
    let result = expand_env_vars("${TEST_MCP_CONFIG_PROTO}://${TEST_MCP_CONFIG_SVC}/v1");
    assert_eq!(result, "https://api.example.com/v1");
    unsafe {
        std::env::remove_var("TEST_MCP_CONFIG_PROTO");
    }
    unsafe {
        std::env::remove_var("TEST_MCP_CONFIG_SVC");
    }
}
...
fn test_expand_no_vars_returns_same_string() {
    assert_eq!(
        expand_env_vars("just a plain string"),
        "just a plain string"
    );
}
