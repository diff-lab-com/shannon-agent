...
pub(crate) fn parse_progress_report(msg: &str) -> Option<ProgressReport> {
...
/// Normalize a blocker reason for streak comparison (case, whitespace).
...
pub(crate) const BLOCKED_AUDIT_TURNS: usize = 3;
...
pub(crate) fn goal_continuation_decision_with_facts(
    ...
    if next.stall_strikes >= crate::repl::state::GOAL_DEFAULT_MAX_STALL_STRIKES {
        return GoalContinuation::PausedNoProgress {
            reason: format!(
