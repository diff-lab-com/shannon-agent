//! Task-based integration tests for Shannon's tool-use pipeline.
//!
//! These tests mock the LLM to return scripted tool_use responses, then verify
//! that Shannon correctly executes tools (file write, bash, edit) and produces
//! the expected results. This tests the full headless pipeline:
//!
//!   prompt -> LLM request -> tool_use response -> tool execution -> LLM request -> text response -> output
//!
//! Run with: cargo test --test cli_task_tests -- --test-threads=1
...
fn create_workspace(name: &str) -> PathBuf {
    let dir = std::env::temp_dir().join(format!("shannon-task-{name}-{}", std::process::id()));
