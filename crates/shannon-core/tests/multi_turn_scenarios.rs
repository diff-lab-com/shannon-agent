...
fn test_multi_turn_planning_has_3_turns() {
    ...
    let user_msgs: Vec<_> = entries.iter()
        .filter(|e| e.get("type").and_then(|t| t.as_str()) == Some("UserMessage"))
        .collect();
    assert_eq!(user_msgs.len(), 3, "Expected 3 user messages in multi-turn planning");

    // Verify turns are 1, 2, 3
    let turns: Vec<usize> = user_msgs.iter()
...
fn test_multi_turn_later_turns_reference_earlier() {
    let entries = parse_fixture("multi_turn_planning.jsonl");
    let llm_responses: Vec<_> = entries.iter()
...
fn test_retry_after_error_has_2_turns() {
    ...
    let user_msgs: Vec<_> = entries.iter()
        .filter(|e| e.get("type").and_then(|t| t.as_str()) == Some("UserMessage"))
        .collect();
    assert_eq!(user_msgs.len(), 2, "Expected 2 user messages (initial + retry)");
}
...
fn test_interactive_debugging_multi_turn() {
    ...
    let user_msgs: Vec<_> = entries.iter()
        .filter(|e| e.get("type").and_then(|t| t.as_str()) == Some("UserMessage"))
        .collect();
    assert!(user_msgs.len() >= 2, "Expected >= 2 user messages in debugging session");

    // Second user message should contain error context
    let second_msg = user_msgs.get(1).unwrap();
    let content = second_msg.get("content").and_then(|c| c.as_str()).unwrap_or("");
    assert!(content.contains("panicked") || content.contains("error") || content.contains("unwrap"),
...
fn test_context_pressure_many_tools() {
    ...
    let tool_calls: Vec<_> = entries.iter()
        .filter(|e| e.get("type").and_then(|t| t.as_str()) == Some("ToolCall"))
        .collect();
    // Should have many reads + edits (high context usage)
    assert!(tool_calls.len() >= 8, "Expected >= 8 tool calls in context pressure, got {}", tool_calls.len());
}
...
fn test_git_workflow_tool_count() {
    ...
    let tool_calls: Vec<_> = entries.iter()
        .filter(|e| e.get("type").and_then(|t| t.as_str()) == Some("ToolCall"))
        .collect();
    assert!(tool_calls.len() >= 3, "Expected >= 3 tool calls in git workflow");
}
...
fn test_multi_turn_request_snapshot() {
    ...
    let entries = parse_fixture("multi_turn_planning.jsonl");
    let requests: Vec<_> = entries.iter()
    ...
    for (i, req) in requests.iter().enumerate() {
        ...
        assert!(!snapshot.is_empty(), "Request {} should render non-empty snapshot", i);
    }
}
...
fn test_cascading_edits_context_growth() {
    ...
    let responses: Vec<_> = entries.iter()
        .filter(|e| e.get("type").and_then(|t| t.as_str()) == Some("LlmResponse"))
        .collect();

    // Multiple LLM responses indicate the model iterated through the edit chain
    assert!(responses.len() >= 3, "Expected >= 3 LLM responses in cascading edits, got {}", responses.len());
}
...
fn test_all_multi_turn_fixtures_have_sequential_turns() {
    ...
    for name in &multi_turn_fixtures {
        ...
        let turns: Vec<usize> = entries.iter()
            .filter_map(|e| {
        ...
        if turns.len() > 1 {
            for window in turns.windows(2) {
                assert_eq!(window[1] - window[0], 1,
                    "{}: turns not sequential: {:?}", name, turns);
            }
        }
    }
}
