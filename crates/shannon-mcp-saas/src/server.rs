...
//! `McpTool` trait from the GitHub module.
...
use crate::github::tools::McpTool;
...
pub async fn run_stdio(tools: &[Box<dyn McpTool>]) -> io::Result<()> {
...
pub fn handle_line(line: &str, tools: &[Box<dyn McpTool>]) -> Option<JsonRpcResponse> {
...
fn dispatch(id: &str, request: &JsonRpcRequest, tools: &[Box<dyn McpTool>]) -> JsonRpcResponse {
...
fn handle_tools_list(id: &str, tools: &[Box<dyn McpTool>]) -> JsonRpcResponse {
...
pub fn print_tool_listing(tools: &[Box<dyn McpTool>]) {
...
mod tests {
    ...
    fn fresh_tools() -> Vec<Box<dyn McpTool>> {
        all_tools_unauth()
    }
    ...
    #[test]
    fn handle_tools_call_invokes_known_tool() {
        let tools = fresh_tools();
        let resp = handle_line(
            r#"{"jsonrpc":"2.0","id":"3","method":"tools/call","params":{"name":"github_list_issues","arguments":{"owner":"diff-lab-com","repo":"shannon-agent"}}}"#,
            &tools,
        )
        .expect("response");
        // Unauthenticated call against a non-routable host will fail
        // with a network error, but the tool dispatch + parse path
        // already proves wire compatibility. The error surfaces
        // through `isError: true`.
        let result = resp.result.expect("result");
        let text = result["content"][0]["text"].as_str().expect("text");
        // Either a JSON success or an error string. We accept both.
        assert!(!text.is_empty());
    }

    #[test]
    fn handle_tools_call_reports_unknown_tool() {
        let tools = fresh_tools();
        let resp = handle_line(
    ...
    #[test]
    fn parse_error_returns_parse_error_code() {
        let tools = fresh_tools();
        let resp = handle_line(r#"not json"#, &tools).expect("response");
