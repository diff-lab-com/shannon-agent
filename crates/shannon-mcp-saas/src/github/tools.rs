...
/// Erase the per-SaaS `McpTool` trait into the `server::ServerTool`
/// trait object so the JSON-RPC loop can dispatch uniformly.
pub fn as_server_tool(tools: Vec<Box<dyn McpTool>>) -> Vec<Box<dyn crate::server::ServerTool>> {
    tools
        .into_iter()
        .map(|t| Box::new(GithubServerTool(t)) as Box<dyn crate::server::ServerTool>)
        .collect()
}

/// Adapter from `Box<dyn McpTool>` to `Box<dyn ServerTool>`.
pub struct GithubServerTool(pub Box<dyn McpTool>);

#[async_trait]
impl crate::server::ServerTool for GithubServerTool {
    fn name(&self) -> &'static str {
        self.0.name()
    }
    fn description(&self) -> &'static str {
