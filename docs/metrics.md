# Shannon Project Metrics

> **Single authoritative metrics source.** Generated by `scripts/gen-metrics.sh`.
> Do not edit by hand — re-run the script after merge to `main`.

## Snapshot

- **Generated (UTC)**: `2026-09-24T02:14:05Z`
- **Branch**: `HEAD`
- **Commit**: `5d87684`
- **Describe**: `5d87684-dirty`

## Summary

| metric | value |
|---|---:|
| Tests (nextest, runnable) | 12526 |
| Tests (source `#[test]`/`#[tokio::test]` attrs) | 12431 |
| Rust source files | 755 |
| Rust LOC (code) | 524620 |
| Workspace members | 22 |
| `cargo clippy --workspace -- -D warnings` | pass |
| `cargo deny check` | pass |

## Test counts

Counts below come from `cargo nextest list --workspace --message-format=json`
(the same set of tests that run in CI). Counts are deduplicated by test id.

| crate | tests | binaries |
|---|---:|---:|
| shannon-core | 3889 | 46 |
| shannon-tools | 1780 | 18 |
| shannon-ui | 1529 | 8 |
| shannon-engine | 1162 | 4 |
| shannon-agents | 907 | 10 |
| shannon-desktop | 885 | 5 |
| shannon-mcp | 581 | 9 |
| shannon-cli | 502 | 9 |
| shannon-commands | 435 | 5 |
| shannon-skills | 189 | 6 |
| shannon-mcp-saas | 185 | 2 |
| shannon-codegen | 100 | 4 |
| shannon-types | 88 | 3 |
| shannon-agent | 65 | 2 |
| shannon-remote | 59 | 1 |
| shannon-api-protocol | 43 | 4 |
| shannon-tool-interface | 42 | 1 |
| shannon-server | 39 | 1 |
| shannon-repomap | 32 | 3 |
| shannon-browser | 9 | 1 |
| shannon-plugin-api | 5 | 1 |
| shannon-stability-attr | 0 | 1 |

## Line counts

Source: `find … -name '*.rs' \| xargs wc -l` (fallback; `cloc` not installed).
Legacy archives and vendored directories are excluded.

| language | files | code |
|---|---:|---:|
| Rust (.rs) | 755 | 524620 |
| TypeScript/TSX | 640 | 106447 |

## Lint / audit status

### `cargo clippy --workspace -- -D warnings`

- **Status**: pass

```text
[1m[33mwarning[0m[1m:[0m shannon-types@0.11.0: Generated ProviderModelConfig JSON Schema at: /home/runner/work/shannon-agent/shannon-agent/crates/shannon-types/schema/provider-model-config.schema.json
[1m[32m   Compiling[0m shannon-desktop v0.11.0 (/home/runner/work/shannon-agent/shannon-agent/desktop)
[1m[32m    Finished[0m \`dev\` profile [unoptimized + debuginfo] target(s) in 23.73s
```

### `cargo deny check`

- **Status**: pass

```text
    [0m[36m│[0m [0m[33m╰[0m[0m[33m──────────────────────────────────────────────────────────────────────────┘ lock entries[0m

advisories [32mok[0m, bans [32mok[0m, licenses [32mok[0m, sources [32mok[0m
```

---

_Regenerate with: `bash scripts/gen-metrics.sh`._
