...
## 11. 实施完成记录（2026-09-06）

本计划 19 项任务已全部在本分支实施并通过逐任务审查与全分支终审：

- **交付**：85+ commits、231 文件、约 4.8 万行；Rust 测试 core 2862 / desktop 726 / server 24，前端 vitest 1665 全绿；逐任务 9 轮修复循环 + 全分支终审 1 轮修复（batch 收件箱标注、预算拒绝回滚、loopback 沙箱接线）均已收口。
- **已合并泳道**：A(batch)/B(权限)/C(IM 渠道)/D(预览)/E(迁移)/F(终端)/G(记忆)/H(面板)/I(配置包)/J(办公 skills)/K(低峰)/L(移动派发)/M(GitHub 触发器)。
- **审查裁定挂起的后续项**：P1-2b（引擎 per-run cwd + 无人值守 profile deny-list 消费）、P1-4b（WS 协议 profile 字段）、I-8（timeout_secs 消费）、I-9（env-dependent 测试排查）；I-10（事件 ACL 潜伏缺陷）已随 T6 修复；P2-4b（桌面记忆注入）已随 T17 交付。
- **合并到 dev 后待办**：重新生成 docs/metrics.md（修复快照头）；在 §7 KPI 表补记 loop/ralph slash 缩减决策（桌面仅 /goal）。
