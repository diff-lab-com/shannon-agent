...
P2 L-tier 启用前置条件:P2-6(perf 通道 /proc 采样)已落地(`7761e4d2`,2026-08-22),**只欠 P2-5(L 任务集)**。本晋级证据证明 [S, M] 段稳定;L 段启用后的成本与回放口径见 §6.2。

### 6.2 L-tier 成本与回放口径(2026-08-22,ed 拍板)

P2-5 启用 L 段前定下的成本策略。核心原则:**回放(record/replay)是回归通道,不是发现通道——live 只买"发现",回归零 token 走回放**。

| # | 决策 | 理由 |
|---|------|------|
| 1 | **毕业证据必须 live**:§6 "全绿含 2 次 L" 仅指 live L 任务,回放不计入 | 回放重走被录制钉死的路径(录成即永成、录败即永败),产物判定 pass/partial/fail 失去意义;放行回放假绿,坡道会被"重放绿"骗过 |
...
10. **(2026-08-22 增补)L-tier 成本与回放口径**:live 只买发现——毕业证据必须 live(§6 口径不变),回放不计入;缓存校准先行(cache_read 占比决定预算压力是否真存在);仍有压力则降频(`l_cadence`)优先于回放替代;回放(回归套件/泄漏 bench/混沌注入)是 L fixture 的附加复用,gate 复跑 ×2 保持 live。详见 §6.2。
