...
**结论（R15 修订）**：10 不应废除而应**降级**为兜底。最终落地值：goal 默认**不限**（0，主动护栏为准，`--max` 显式启用兜底）；`/ralph` 默认 **100**（任务迭代粒度小、合法轮数多，对齐 OpenHands）；`/loop` 维持默认不限（勘误：loop 从未有 10 的默认）。风险显式化：loop = 无终止判据 + 无主动护栏 + 默认不限，是三循环中最危险的组合——anti-spin/stall 移植到 loop/ralph（统一 LoopGuard）已升级为**必要跟进项**。
...
| ralph 默认 10 → 100；goal 默认 25 → 0（不限）；loop 维持不限（勘误） | 中 | CHANGELOG + 循环启动消息显示当前上限与护栏说明；goal 的 --max 帮助文案改为「显式兜底，默认不限」 |
...
2. **ralph/loop 默认放宽时机**：✅ P2.1/P2.2 先合入后再放宽（已按此执行）；R15 进一步修正落地值：ralph 10→100、goal 25→0、loop 勘误维持不限。
3. **ralph 关键词兼容策略**：✅ OR 兼容期 + 弃用时间表（关键词仅认可末行以消除 substring 误触；下个 minor 移除关键词路径）。
4. **goal 预算默认值**：✅ 默认不设 `--budget`、仅告警；不引入隐式终止。
5. **`--max` 的最终归宿**：✅ 已定（用户拍板，R15）——goal 默认 **0（不限）**，`--max N` 仅显式限定；loop/ralph 采用更大的默认轮数（ralph=100，对齐 OpenHands），任务迭代轮的粒度远小于 goal 轮，25 远远不够。
