...
1. **P2.0 是否先行单独发**？建议：是（生产栈溢出隐患，与护栏体系解耦）。
2. **ralph/loop 默认 10 → 25**：同意放宽，还是维持 10 直到 P2.1/P2.2 合入？（方案假设后者先合、再放宽，若倾向保守可把默认值变更并入 P2.6 尾部）
3. **ralph 关键词兼容策略**：OR 兼容期 + 弃用时间表（推荐），还是直接替换？
4. **goal 预算默认值**：P2.3 落地后，`--budget` 不设时是否跟随全局 `monthly_budget`（BillingConfig 已有）按比例（如 20%）设置 goal 级软上限？还是默认不设、仅告警？推荐后者（避免隐式终止）。
5. **`--max` 的最终归宿**：P2.3/P2.5 全部落地后，goal 兜底上限是否提到 100（对齐 OpenHands）或默认 0（对齐 Claude Code/Codex 的"无轮数上限"）？建议届时再定，本方案不预先承诺。
6. **`LoopGuard` 落位**：`shannon-core`（推荐，headless/引擎侧未来可用）vs `shannon-ui`（改动面最小）。
