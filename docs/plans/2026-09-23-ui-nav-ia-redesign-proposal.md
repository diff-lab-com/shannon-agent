# Shannon Desktop 导航与信息架构综合改进方案（提案 v1，待评审）

> **状态**：提案（分析 + 方案，不含代码改动），供 ericdong 评审后排期。
...
| T8 | GoalRunPanel/BatchRunPanel 的「监控」职责评估：运行卡保留在「自动化-运行」tab（管理员视角），rail 已有 goal badge（`SidebarSessions.tsx:653-686`）承担个人监控 | /tasks 不再是监控必经之路 |
...
| 用量 / 指挥台（dev） | — | 不变（观测碎片化问题另立工作项，见 §5） | — |
...
**开放问题（请评审人裁决）**：
1. 阶段二 T8：GoalRunPanel/BatchRunPanel 是保留在「自动化-运行」还是降级为 rail badge + 抽屉？（建议：前者，管理员视角有价值）
2. 技能提案的主审查面放在「扩展-待处理」还是「收件箱」？（§3.3 X1 建议：**扩展-待处理为主面**，收件箱条目点击跳转过去——审批需要 manifest 上下文，收件箱卡太薄；若你倾向「收件箱为主面」，X1 的聚合方向反转即可，工作量相同）
3. 「智能」镜头是否升为默认？（P-U4 建议：不强改，记住上次选择）
4. 观测面碎片化（/usage、/timeline、OPC 仪表盘、SessionUsageDialog 五处成本/token 呈现）本轮未纳入——是否另立「运行观测」专项？
5. /opc（指挥台）长期定位（dev-only 保留 / 并入自动化 / 退役）？
