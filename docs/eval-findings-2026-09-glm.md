...
### 4.6 TB2.1 改进验收复测三部曲（2026-09-06/07）——含方法学教训

| 轮 | 配置 | 结果 | 判定 |
|---|---|---|---|
| v2 | 改进二进制 + 看门狗 180s + 6 并发 + `-p - < file`（**坏**：字面 `-` 当 prompt） | 0/85（1h23 假完成） | ❌ 无效：prompt 全损 |
| v3 | 改进二进制 + 看门狗 180s→(中途改)420s + 6 并发 + `--prompt=$(cat)`（已验证逐字节） | 17/89 (19.1%) | ❌ 无效：三因叠加（6 并发限流 rc=4 死亡 + A2 收工子句过度泛化 + 看门狗阈值仍低于部分静默） |
| v4 | **P1d 等条件**（3 并发 + 无看门狗）+ 改进二进制（含 R1 correctness-first 重写） | 发车后终止（用户决策） | ⏸️ 未产生数据 |

教训（已固化进 docs/backlog.md §一 L1-L4）：
- L1 一次只改一个变量；L2 看门狗阈值 > 模型最大思考静默（GLM-5.3 实测 312s）；
  L3 发车核验断言 start 事件 prompt 字段；L4 prompt 措辞的单题证据永不采信。
- **R1/R2 已落地**（`f6bac100`）：A2 收工子句回退为 correctness-first + 完成门禁保留；
  B.7 strikes 默认改 opt-in。
- **TB2.1 改进效果目前未验收**（SWE50 37/50 为已验收的改进证据）；
  复测入口：`docs/backlog.md` §四 T1。
