...
- **F18（已修复，2026-09-18 第三批）**：`shannon-ui` 的 9 个存量失败全部清零——
  ① `test_generate_tree_file_not_dir` 夹具 `/etc/hostname` 是 Linux 专用路径，改用
  双平台存在的 `/etc/hosts`；② `handle_ci` 的 help/无参 usage 分支移到 gh 检查之前
  （未装 gh 的用户现在也能看 `/ci help`，产品 UX 改进），repl_ci 三件套转绿；
  ③ remote 三件套的 `HomeGuard` 恢复值从 "/" 改为原始 HOME 并持有共享 `ENV_LOCK`
  （旧实现把 HOME 恢复成 "/"，污染后续所有读 HOME 的测试）；④ goal.rs /
  loop_engine.rs 的同款 HomeGuard 一并收编到共享实现；⑤ `load_permission_rules`/
  `rewind`/`complete_file_path_tilde` 等 HOME/cwd 敏感测试持 `test_env::env_lock()`
  运行。最终 shannon-ui 全量 **1447/0 失败**（连续 3 轮稳定），clippy 归零。
# macOS 真机验证结果（A2/A3，2026-09-10）
...
- **F17（设计记录，未改）**：headless 查询在权限门返回 Prompt（需询问）且无审批
  通道时按 "assume auto-allow" 兜底（engine.rs）；仅 Critical 被硬拒。若要求
  headless 对 High-risk 一律拒绝需另立项（会改变 CI/eval 行为）。
- **F18（已于 2026-09-18 第三批修复，见上文 F18 条）**：`shannon-ui` 9 个存量
  失败——remote handler 三件套、repl_ci 三件套、at_reference、rewind、
  permission-rules。根因三类：Linux 专用夹具、`/ci` 帮助分支被 gh 检查挡住、
  HOME/cwd 全局状态竞态（HomeGuard 恢复 "/" + 无串行化）。
