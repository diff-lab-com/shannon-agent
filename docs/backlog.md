...
**需要实测定谳**：行为改动在 TB2.1 的净方向（T1 的 -8 扣除网络噪声后余 ~9pp 未归因）。
设计：2×2 析因（提示 P1d/R1 × nudge on/off），22 题分层样本，3 并发，看门狗关，
全部 cell 用「当前引擎代码」仅切换提示文本与 nudge env（避免 P1d 整体二进制的混杂）。
其中 (R1,on) cell 复用 T1 子集、(P1d,off) 需构建 bin-p1d-prompt（仅回退提示文本）。
