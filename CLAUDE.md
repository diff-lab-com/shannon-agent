...
- **Desktop app**: Substantial Tauri app (workspace member `desktop/`, crate `shannon-desktop`, ~25K Rust LOC) with command modules for chat/config/connections/billing/agents/mcp/memory. Provider/credential read+write paths are unified with the CLI via the engine `ProviderConfigService` (ADR-0005 Phase 2 ✅ done) + `ProviderReadSnapshot` (ADR-0009) + TD-4 `ProviderConnection` DTO alignment; `has_api_key` is derived from the shared credential store. Remaining tail (small): per-tier/per-fallback editors + `SHANNON_*_PROVIDERS` allowlist toggle in the Add Provider modal (frontend), and one-shot legacy `providers.json` migration-code removal after a release grace period.
