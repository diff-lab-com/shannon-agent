...
function ReassignModal({
  ...
  const submit = async () => {
    ...
    if (!trimmed) { setError('Pick an agent'); return }
    setSaving(true); setError(null)
    try {
      await api.updateTask({ id: target.taskId, assignee: trimmed })
      toast.success(`Reassigned "${target.taskTitle}" to ${trimmed}`)
      onSubmitted()
    } catch (e) {
    ...
    >
      ...
      >
        ...
        <p className="font-body-md text-on-surface-variant">
          Move <strong className="text-on-surface">{target.taskTitle}</strong> off{' '}
          <strong className="text-on-surface">{target.agentName}</strong>.
        </p>
        <div className="space-y-sm">
