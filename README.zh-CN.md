...
- **每一行代码可审计** —— Apache-2.0，无黑盒。<!-- metrics:start:intro -->每一行代码都可审计，每一个行为都经过 **12,526** 个自动化测试验证。<!-- metrics:end:intro -->
...
<!-- metrics:start:diffrow -->| 测试覆盖 | **12,526** 个测试，覆盖 22 个 workspace 成员 | 不适用（闭源） | 各不相同 |<!-- metrics:end:diffrow -->
...
| Crate | 测试数 | 职责 |
|-------|--------|------|
| `shannon-core` | 3,997 | API 客户端、查询引擎、权限、工具、状态 |
| `shannon-tools` | 1,765 | 工具实现：文件操作、Git、搜索、Notebook |
| `shannon-ui` | 1,525 | 终端 UI、REPL、组件、渲染 |
| `shannon-engine` | 1,179 | LLM API 客户端、流式适配、压缩/上下文预算、权限 |
| `shannon-agents` | 902 | 多 Agent 协作：团队、工作树隔离 |
| `shannon-desktop` | 859 | Tauri 桌面应用外壳与命令 |
| `shannon-mcp` | 580 | MCP 协议：传输层、服务器、客户端、进程池 |
| `shannon-cli` | 502 | CLI 入口（`shannon` 二进制） |
| `shannon-commands` | 422 | 内置斜杠命令 |
| `shannon-skills` | 189 | 技能框架：发现、加载、执行 |
...
| `shannon-server` | 28 | HTTP API 服务器（`shannon serve`） |
| `shannon-browser` | 6 | — |
