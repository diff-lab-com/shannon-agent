...
- **每一行代码可审计** —— Apache-2.0，无黑盒。<!-- metrics:start:intro -->每一行代码都可审计，每一个行为都经过 **12,565** 个自动化测试验证。<!-- metrics:end:intro -->
...
<!-- metrics:start:diffrow -->| 测试覆盖 | **12,565** 个测试，覆盖 22 个 workspace 成员 | 不适用（闭源） | 各不相同 |<!-- metrics:end:diffrow -->
